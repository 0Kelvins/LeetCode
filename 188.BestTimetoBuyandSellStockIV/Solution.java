...
public class Solution {
    ...
    private int deal(int[] prices, int i, int t, boolean s) {
        ...
        if (cache.containsKey(key)) {
            ...
            max = s ? (deal(prices, i + 1, t - 1, false) + prices[i]) // 卖出
                    : (deal(prices, i + 1, t, true) - prices[i]);     // 买入
            max = Math.max(deal(prices, i + 1, t, s), max);           // 持有观望及无股观望
    ...
    public static void main(String[] args) {
        ...
        int[] k = { 2, 2, 1, 2, 29 };
        int[][] prices = { { 2, 4, 1 }, { 3, 2, 6, 5, 0, 3 }, { 0, 10, 6, 12 }, { 0, 10, 6, 12 },
        ...
        for (int i = 0; i < k.length; i++) {
            System.out.println(s.maxProfit(k[i], prices[i]));
        }
    }
}